...
export const handleSubscriptionCreated: Task = async (payload, helpers) => {
  ...
  if (event.type !== "COMM.SUBSCRIPTION_CREATED") {
    helpers.logger.warn(
      `handle_subscription_created received unexpected event type: ${event.type}`,
    );
    return;
  }

  helpers.logger.info(
    `subscription created: subscriptionId=${event.payload.subscriptionId} principalId=${event.payload.principalId} ` +
  ...
  if (
    ...
    helpers.logger.warn(
      `subscription created with unsupported entityType=${event.payload.entityType}`,
    );
    return;
  }
  ...
  await createInboxItem(helpers, {
    ...
    body: `You are now following ${event.payload.entityType} ${event.payload.entityId}.`,
  });
};
