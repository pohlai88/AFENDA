...
export async function hrSubmitApplicationRoutes(app: FastifyInstance) {
  ...
  typed.post("/hrm/applications", { schema: { description: "Submit application.", tags: ["HRM", "Recruitment"], security: [{ bearerAuth: [] }, { devAuth: [] }], body: BodySchema, response: { 201: ResponseSchema, 400: ApiErrorResponseSchema, 401: ApiErrorResponseSchema, 403: ApiErrorResponseSchema, 404: ApiErrorResponseSchema, 409: ApiErrorResponseSchema, 500: ApiErrorResponseSchema } } }, async (req, reply) => {
    const orgId = requireOrg(req, reply); if (!orgId) return;
    const auth = requireAuth(req, reply); if (!auth) return;
    const result = await submitApplication(app.db, orgId, auth.principalId, req.correlationId, req.body);
    if (!result.ok) {
      const status = result.error.code === "HRM_CONFLICT" ? 409 : result.error.code === "HRM_CANDIDATE_NOT_FOUND" || result.error.code === "HRM_REQUISITION_NOT_FOUND" ? 404 : 400;
