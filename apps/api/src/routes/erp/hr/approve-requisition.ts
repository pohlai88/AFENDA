...
import { z } from "zod";
import { ApproveRequisitionResultSchema } from "@afenda/contracts";
...
const ResponseSchema = makeSuccessSchema(ApproveRequisitionResultSchema);

export async function hrApproveRequisitionRoutes(app: FastifyInstance) {
  const typed = app.withTypeProvider<ZodTypeProvider>();
  typed.post(
    "/hrm/requisitions/:requisitionId/approve",
    {
      schema: {
    ...
    async (req, reply) => {
      ...
      if (!auth) return;
      const result = await approveRequisition(app.db, orgId, auth.principalId, req.correlationId, {
