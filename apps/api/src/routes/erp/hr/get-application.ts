...
import { z } from "zod";
import { ApplicationViewSchema } from "@afenda/contracts";
...
const ResponseSchema = makeSuccessSchema(ApplicationViewSchema);

export async function hrGetApplicationRoutes(app: FastifyInstance) {
  const typed = app.withTypeProvider<ZodTypeProvider>();
  typed.get(
    "/hrm/applications/:applicationId",
    {
      schema: {
    ...
    async (req, reply) => {
      ...
      if (!data) {
        return reply
          .status(404)
          .send({
            error: { code: "HRM_APPLICATION_NOT_FOUND", message: "Application not found" },
