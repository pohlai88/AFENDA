...
import { z } from "zod";
import { listBenefitEnrollments } from "@afenda/core";
import { ApiErrorResponseSchema, requireAuth, requireOrg } from "../../../helpers/responses.js";

const QuerySchema = z.object({
  employmentId: z.string().uuid().optional(),
  enrollmentStatus: z.enum(["active", "suspended", "terminated", "pending"]).optional(),
  limit: z.coerce.number().int().min(1).max(200).optional(),
  offset: z.coerce.number().int().min(0).optional(),
});

export async function hrListBenefitEnrollmentsRoutes(app: FastifyInstance) {
  ...
  typed.get(
    ...
    async (req, reply) => {
      ...
      const rows = await listBenefitEnrollments(app.db, orgId, {
