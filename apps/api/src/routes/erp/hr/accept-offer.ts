...
import { z } from "zod";
import { AcceptOfferCommandSchema, AcceptOfferResultSchema } from "@afenda/contracts";
...
const BodySchema = AcceptOfferCommandSchema.omit({ idempotencyKey: true, offerId: true });
const ResponseSchema = makeSuccessSchema(AcceptOfferResultSchema);

export async function hrAcceptOfferRoutes(app: FastifyInstance) {
  const typed = app.withTypeProvider<ZodTypeProvider>();
  typed.post(
    "/hrm/offers/:offerId/accept",
    {
      schema: {
    ...
    async (req, reply) => {
      ...
      if (!auth) return;
      const result = await acceptOffer(app.db, orgId, auth.principalId, req.correlationId, {
