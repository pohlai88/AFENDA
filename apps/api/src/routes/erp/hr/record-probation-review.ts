...
export async function hrRecordProbationReviewRoutes(app: FastifyInstance) {
  ...
  typed.post("/hrm/probation-reviews", { schema: { description: "Record probation review.", tags: ["HRM", "Onboarding"], security: [{ bearerAuth: [] }, { devAuth: [] }], body: BodySchema, response: { 201: ResponseSchema, 400: ApiErrorResponseSchema, 401: ApiErrorResponseSchema, 403: ApiErrorResponseSchema, 404: ApiErrorResponseSchema, 500: ApiErrorResponseSchema } } }, async (req, reply) => {
    const orgId = requireOrg(req, reply); if (!orgId) return;
    const auth = requireAuth(req, reply); if (!auth) return;
    const result = await recordProbationReview(app.db, orgId, auth.principalId, req.correlationId, req.body);
    if (!result.ok) {
