...
import { StartSeparationCommandSchema, StartSeparationResultSchema } from "@afenda/contracts";
...
const BodySchema = StartSeparationCommandSchema.omit({ idempotencyKey: true });
const ResponseSchema = makeSuccessSchema(StartSeparationResultSchema);

export async function hrStartSeparationRoutes(app: FastifyInstance) {
  const typed = app.withTypeProvider<ZodTypeProvider>();
  typed.post(
    "/hrm/separation-cases",
    {
      schema: {
        description: "Start separation case.",
        tags: ["HRM", "Onboarding"],
