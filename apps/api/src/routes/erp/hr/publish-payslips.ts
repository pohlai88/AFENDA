...
import { publishPayslips } from "@afenda/core";
import { z } from "zod";
import {
...
export async function hrPublishPayslipsRoutes(app: FastifyInstance) {
  const typed = app.withTypeProvider<ZodTypeProvider>();

  typed.post(
    "/hrm/payroll/runs/:payrollRunId/publish-payslips",
    {
      schema: {
        description: "Publish payslips for an approved payroll run.",
        tags: ["HRM", "Payroll"],
        ...
        const status =
          result.error.code === "HRM_PAYROLL_RUN_NOT_FOUND"
            ...
            : result.error.code === "HRM_CONFLICT" ||
                result.error.code === "HRM_PAYSLIPS_ALREADY_PUBLISHED"
              ? 409
              : 400;
