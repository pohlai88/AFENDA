...
import { z } from "zod";
import { PositionIncumbencyViewSchema } from "@afenda/contracts";
...
const ResponseSchema = makeSuccessSchema(PositionIncumbencyViewSchema);

export async function hrGetPositionIncumbencyRoutes(app: FastifyInstance) {
  const typed = app.withTypeProvider<ZodTypeProvider>();
  typed.get(
    "/hrm/positions/:positionId",
    {
      schema: {
        description: "Get position incumbency.",
    ...
    async (req, reply) => {
      ...
      if (!data) {
        return reply
          .status(404)
          .send({
            error: { code: "HRM_POSITION_NOT_FOUND", message: "Position not found" },
