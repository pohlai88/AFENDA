...
import { listLeaveBalances } from "@afenda/core";
import { z } from "zod";
import {
...
const QuerySchema = z.object({
  ...
  accrualPeriod: z.string().min(1).max(20).optional(),
...
export async function hrListLeaveBalancesRoutes(app: FastifyInstance) {
  ...
  typed.get(
    ...
    async (req, reply) => {
      ...
      const data = await listLeaveBalances(app.db, { orgId, ...req.query });
