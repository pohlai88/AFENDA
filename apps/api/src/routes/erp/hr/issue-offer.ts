...
import { IssueOfferCommandSchema, IssueOfferResultSchema } from "@afenda/contracts";
...
const BodySchema = IssueOfferCommandSchema.omit({ idempotencyKey: true });
const ResponseSchema = makeSuccessSchema(IssueOfferResultSchema);
      ...
      if (!result.ok) {
        const status =
          result.error.code === "HRM_CONFLICT"
            ? 409
            : result.error.code === "HRM_APPLICATION_NOT_FOUND"
              ? 404
              : 400;
        return reply
          .status(status)
