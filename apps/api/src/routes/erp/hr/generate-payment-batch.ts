...
import { generatePaymentBatch } from "@afenda/core";
import { z } from "zod";
import {
...
export async function hrGeneratePaymentBatchRoutes(app: FastifyInstance) {
  const typed = app.withTypeProvider<ZodTypeProvider>();

  typed.post(
    "/hrm/payroll/runs/:payrollRunId/generate-payment-batch",
    {
      schema: {
        description: "Generate payment batch from an approved payroll run.",
        tags: ["HRM", "Payroll"],
    ...
    async (req, reply) => {
      ...
      const result = await generatePaymentBatch(
        app.db,
        orgId,
        auth.principalId,
        req.correlationId,
        ...
        const status =
          result.error.code === "HRM_PAYROLL_RUN_NOT_FOUND"
            ...
            : result.error.code === "HRM_CONFLICT" ||
                result.error.code === "HRM_PAYMENT_BATCH_ALREADY_EXISTS"
              ? 409
              : 400;
