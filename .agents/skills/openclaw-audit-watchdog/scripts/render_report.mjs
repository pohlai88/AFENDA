...
function readJsonSafe(p, label) {
  ...
  try {
    ...
    return {
      findings: [],
      summary: {},
      error: `${label} parse failed: ${e?.message || String(e)}`,
    };
  }
}
...
function render({ audit, deep, label, suppressedFindings = [] }) {
  ...
  lines.push(`Time: ${now}`);
  lines.push(
    `Summary: ${summary.critical ?? 0} critical · ${summary.warn ?? 0} warn · ${summary.info ?? 0} info`,
  );
...
if (audit.findings) {
  audit.findings = activeFindings.filter((f) => (audit.findings || []).some((orig) => orig === f));
...
if (deep.findings) {
  deep.findings = activeFindings.filter((f) => (deep.findings || []).some((orig) => orig === f));
  // Recalculate summary counts after filtering
  deep.summary = {
