...
**Why:** Tailwind v4 strips CSS outside `@theme`/`@layer`, but `:root` must be
at root level.
...
**Why:** Variables already contain `hsl()`, double-wrapping creates
`hsl(hsl(...))`.
...
**Why:** With proper CSS variable setup, `bg-primary` automatically responds to
theme.
