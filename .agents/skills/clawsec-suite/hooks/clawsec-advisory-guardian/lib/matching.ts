  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (
        ...
        typeof parsedSkillJson.name === "string" &&
        parsedSkillJson.name.trim()
      ) {
...
export function affectedSpecifierMatchesSkill(
  rawSpecifier: string,
...
export function looksMalicious(advisory: Advisory): boolean {
  ...
  const combined =
    `${advisory.title ?? ""} ${advisory.description ?? ""} ${advisory.action ?? ""}`.toLowerCase();

  if (type === "malicious_skill" || type === "malicious_plugin") return true;
  if (/\b(malicious|exfiltrat(e|ion)|backdoor|trojan|credential theft|stealer)\b/.test(combined))
    return true;
  return false;
}
...
export function looksRemovalRecommended(advisory: Advisory): boolean {
  const combined =
    `${advisory.action ?? ""} ${advisory.title ?? ""} ${advisory.description ?? ""}`.toLowerCase();
...
export function buildAlertMessage(matches: AdvisoryMatch[], installRoot: string): string {
  ...
  const removalMatches = matches.filter(
    (entry) => looksMalicious(entry.advisory) || looksRemovalRecommended(entry.advisory),
  );
  if (removalMatches.length > 0) {
    ...
    lines.push("");
    lines.push(
      "Recommendation: one or more matches indicate potentially malicious or unsafe skills.",
    );
    lines.push(
      "Best practice: remove or disable affected skills only after explicit user approval.",
    );
    lines.push(
    ...
    lines.push(
      `Approval needed: ask the user to approve removal of: ${impactedSkills.join(", ")}.`,
    );
