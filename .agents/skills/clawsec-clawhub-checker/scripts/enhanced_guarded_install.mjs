...
function parseArgs(argv) {
  ...
  if (!/^[a-z0-9][a-z0-9-]*$/.test(parsed.skill)) {
    throw new Error(
      "Invalid --skill value. Must start with a letter or digit, followed by lowercase letters, digits, and hyphens.",
    );
  }
  if (
    parsed.version &&
  ...
  if (
    parsed.reputationThreshold < 0 ||
    parsed.reputationThreshold > 100 ||
    Number.isNaN(parsed.reputationThreshold)
  ) {
...
async function runOriginalGuardedInstall(args) {
  ...
  try {
    ...
    throw new Error(
      `Original guarded_skill_install.mjs not found at ${originalScript}. Is clawsec-suite installed?`,
    );
  }

  // Pass through environment without modification
  // The original guarded_skill_install.mjs handles --confirm-advisory properly
  const child = spawnSync("node", [originalScript, ...args.originalArgs], {
    stdio: "inherit",
    env: process.env,
    cwd: suiteDir,
  });
...
async function main() {
  try {
    ...
    if (!args.confirmReputation) {
      console.log(
        `Checking ClawHub reputation for ${args.skill}${args.version ? `@${args.version}` : ""}...`,
      );

      const reputationResult = await checkClawhubReputation(
        args.skill,
        args.version,
        args.reputationThreshold,
      );
      ...
      if (!reputationResult.safe) {
        ...
        console.error(
          `Skill "${args.skill}" has low reputation score: ${reputationResult.score}/100`,
        );
        ...
        console.error(
          `  node ${process.argv[1]} --skill ${args.skill}${args.version ? ` --version ${args.version}` : ""} --confirm-reputation`,
        );
        console.error("");
        console.error("=".repeat(80));
