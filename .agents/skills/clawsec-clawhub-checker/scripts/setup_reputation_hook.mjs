...
async function main() {
  ...
  try {
    ...
    const reputationModuleSrc = path.join(
      checkerDir,
      "hooks",
    ...
    if (!handlerContent.includes(importLine)) {
      ...
      if (importIndex === -1) {
        throw new Error(
          "Could not find import statements in handler.ts. Manual integration required.",
        );
      }

      const lineEndIndex = handlerContent.indexOf("\n", importIndex);
      handlerContent =
        handlerContent.slice(0, lineEndIndex + 1) +
        `${importLine}\n` +
...
`;
      handlerContent =
        handlerContent.slice(0, insertIndex) +
        reputationCheckCode +
        handlerContent.slice(insertIndex);
      handlerChanged = true;
    } else {
...
`;
    ...
    console.log(
      "  node scripts/guarded_skill_install_wrapper.mjs --skill <name> [--version <ver>]",
    );
