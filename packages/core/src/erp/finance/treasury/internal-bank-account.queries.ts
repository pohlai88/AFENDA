...
export class InternalBankAccountQueries {
   ...
   */
  async getById(orgId: string, accountId: string): Promise<InternalBankAccountEntity | null> {
   ...
   */
  async findByCode(orgId: string, code: string): Promise<InternalBankAccountEntity | null> {
    const result = await this.deps.db
      .select()
      .from(treasuryInternalBankAccountTable)
      .where(
