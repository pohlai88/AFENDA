...
export async function submitDraftInvoice(
  ...
  if (!TRANSITIONS[inv.status as InvoiceStatus]?.includes("submitted")) {
    return {
      ok: false,
      error: {
        code: "AP_INVOICE_INVALID_STATUS_TRANSITION",
        message: `Cannot submit invoice in status '${inv.status}' (must be draft)`,
        meta: { currentStatus: inv.status },
      },
    };
  }

  const draftCheck = await hasActiveHolds(db, orgId, invoiceId);
  if (draftCheck) {
    return {
      ok: false,
      error: {
        code: "AP_INVOICE_HAS_ACTIVE_HOLDS",
        message: "Cannot submit invoice with active holds",
        meta: { invoiceId },
      },
    };
  }
  ...
  const auditEntry: {
    ...
    details: {
      ...
      amountMinor: inv.amountMinor.toString(),
  ...
  // Run validation (duplicate detection + 3-way match) and create holds if needed
  await validateInvoice(db, ctx, policyCtx, correlationId, invoiceId);

  return { ok: true, data: { id: invoiceId, invoiceNumber: inv.invoiceNumber } };
}

// ── Submit (create + submit in one) ───────────────────────────────────────────
...
export async function approveInvoice(
  ...
  // Active hold check — blocks approval until all holds are released
  const hasHolds = await hasActiveHolds(db, orgId, invoiceId);
  if (hasHolds) {
    return {
      ok: false,
      error: {
        code: "AP_INVOICE_HAS_ACTIVE_HOLDS",
        message: "Invoice has active holds and cannot be approved until they are released",
        meta: { invoiceId },
      },
    };
  }
...
// ── Bulk Approve / Reject / Void ────────────────────────────────────────────

export type BulkInvoiceResult = { ok: number; failed: number; failedIds: InvoiceId[] };
...
export async function bulkApproveInvoices(
  ...
  const failedIds: InvoiceId[] = [];
  let ok = 0;
  for (const id of invoiceIds) {
    const result = await approveInvoice(db, ctx, policyCtx, correlationId, id, reason);
    if (result.ok) ok++;
...
export async function bulkRejectInvoices(
  ...
  let ok = 0;
  for (const id of invoiceIds) {
    const result = await rejectInvoice(db, ctx, policyCtx, correlationId, id, reason);
    if (result.ok) ok++;
    else failedIds.push(id);
  }
  return { ok, failed: failedIds.length, failedIds };
...
export async function bulkVoidInvoices(
  ...
  let ok = 0;
  for (const id of invoiceIds) {
    const result = await voidInvoice(db, ctx, policyCtx, correlationId, id, reason);
    if (result.ok) ok++;
    else failedIds.push(id);
  }
  return { ok, failed: failedIds.length, failedIds };
