...
| `tracing.ts`       | `instrumentService(namespace, fns)`, `extractRequestAttrs(req)`, `setActiveSpanAttributes(attrs)`                                                                                                                                                                      | Auto-instrumentation for domain services. Wraps every exported function with an OTel span; span names + attributes deterministically derived from function names + argument shapes (duck-typing). No manual annotation needed. `@opentelemetry/api` import — no-op when SDK not registered.                                                                                                                        |
| `otel-insights.ts` | `generateInsights(config?)` → `InsightReport`, `normalise(traces)`, analyzers: `slowOps`, `errorHotspots`, `securitySignals`, `reliability`, `throughput`, `dataQuality`                                                                                               | OTel Insight Factory. Reads traces from Jaeger Query API, runs 6 pluggable analyzers, and produces severity-ranked actionable recommendations. Zero OTel SDK dependency — reads FROM Jaeger, not into it.                                                                                                                                                                                                          |
